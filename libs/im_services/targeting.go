...
func Player(svc dynamodbiface.DynamoDBAPI, playerId int) ([]byte, error) {

	result, err := QetAllItems(svc, DEFAULT_DYNAMO_TABLE, "player_id", playerId)
	if err != nil {
		return nil, err
	}
	...
	if len(players) == 1 {
		playerJson, err := unpackageDynamoHash(players[0], getSchemaHash())
		if err != nil {
			return nil, err
		}
		...
		countDays, err := dateDiffer(players[0]["account_created_at"])
		if err != nil {
		...
		countDays, err = dateDiffer(players[0]["session_last"])
		if err != nil {
...
func dateDiffer(value interface{}) (int, error) {
	if value != nil && isDate(value.(string)) {
		t, _ := time.Parse("2006-01-02", value.(string))
		today := time.Now()
		diff := today.Sub(t)
		return int(diff.Hours() / 24), nil
	} else if value != nil && isTime(value.(string)) {
...
func unpackageDynamoHash(playerData map[string]interface{}, schemaHash map[string]string) ([]byte, error) {
