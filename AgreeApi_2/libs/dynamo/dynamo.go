...
func QueryByIndexN2(svc dynamodbiface.DynamoDBAPI, table, index, fieldName, fieldName_2, fieldValue string, fieldValue_2 string,
	...
	result, err := svc.Query(&dynamodb.QueryInput{
		ExpressionAttributeNames: map[string]*string{
			"#PK": aws.String(fieldName),
			"#SK": aws.String(fieldName_2),
		},
		ExpressionAttributeValues: map[string]*dynamodb.AttributeValue{
			...
			":skValue": {
				S: aws.String(fieldValue_2),
			},
		},
		KeyConditionExpression: aws.String("#PK = :pkValue"),
		FilterExpression:       aws.String("#SK = :skValue"),
		TableName:              aws.String(table),
		IndexName:              aws.String(index), // Optional, if querying a secondary index
...
func GetAllItems(svc dynamodbiface.DynamoDBAPI, table, fieldName string, fieldValue int) (*[]dynamodb.QueryOutput, error) {
	if table == "" || fieldName == "" {
		return nil, nil
	}
...
func QetAllItemsS(svc dynamodbiface.DynamoDBAPI, table, fieldName string, fieldValue string) (*[]dynamodb.QueryOutput, error) {
	if table == "" || fieldName == "" {
		return nil, nil
	}
