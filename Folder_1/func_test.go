...
// TESTING 11
func TestPrimeTimeToDeliver(t *testing.T) {
	type test struct {
		name              string
		playerID          string
	...
	tests := []test{
		{
			name:     " Empty schedule days ",
			playerID: "123",
			campaignTableData: CampaignTable{
				//ScheduleTime: sql.NullInt32{Int32: 1, Valid: true},
		...
		{
			name : " Testing GetValidPlayerId function ",
			playerID: "",
			campaignTableData: CampaignTable{
				//ScheduleTime: sql.NullInt32{Int32: 20, Valid: true},
				ScheduleDays: ScheduleDays{"Saturday", "Sunday"},
			},
			result: false,
			err: fmt.Errorf("player_id can not be null"),
		},
		{
			playerID: "456",
			campaignTableData: CampaignTable{
				//ScheduleTime: sql.NullInt32{Int32: 18, Valid: true},
				ScheduleDays: ScheduleDays{"Sunday", "Monday", "Friday"},
			},
			result: false,
			err:    nil,
		},
	...
	for _, test := range tests {
		result, err := PrimeTimeToDeliver(test.playerID, &test.campaignTableData)
		if result != test.result || err != test.err {
			t.Errorf("PrimeTimeToDeliver(%s, %v) = (%v, %v), want = (%v, %v)", test.playerID, test.campaignTableData, result, err, test.result, test.err)
		}
	}
...
func TestUserTimeToDeliver(t *testing.T) {
	testCases := []testData{
		...
		},
		{
			...
			//currentTime:    time.Date(2022, time.January, 8, 16, 0, 0, 0, time.UTC), // Saturday 4:00pm
			expectedPass:   true,
			expectedError:  fmt.Errorf("player_id can not be null"),
		},
		{
			playerID:       "3fhgsdfh",
			scheduleDays:   ScheduleDays{},
			...
			//currentTime:    time.Date(2022, time.January, 8, 16, 0, 0, 0, time.UTC), // Saturday 4:00pm
			expectedPass:   true,
			expectedError:  fmt.Errorf("player_id must be of type int"),
		},
	}
...
// // 	for _, tc := range testCases {467:9: expected 'package', found 'EOF'
