...
func main() {
	...
	// storing the parsed data into struct
	var msg DynamoInboxStoreItem
	err := json.Unmarshal([]byte(empJson), &msg)
	


	//Reading each value by its key
	
	msg.PushMessageStatus = fmt.Sprintln(push_messages["status"]) 
	msg.PushMessageProviderMessageID = fmt.Sprint(push_messages["provider_message_id"]) 
	msg.PushMessageProvider = fmt.Sprint(push_messages["push_provider"]) 
	msg.PushMessageCreatedAt = fmt.Sprint(push_messages["created_at"]) 
	msg.PushMessageUpdatedAt = fmt.Sprint(push_messages["updated_at"]) 
	msg.PushMessageGUID = fmt.Sprint(push_messages["guid"]) 
	...
	// 	PushMessageCreatedAt: push_messages["created_at"],
	// 	PushMessageUpdatedAt: push_messages["updated_at"],
	// 	PushMessageGUID: push_messages["guid"]
	// }
