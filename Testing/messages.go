...
// BONUSTEMPLATE AND MESSAGE TEMPLATE STRUCT IS MISSING
...
// Messagge template struct is missing
...
// STRUCT TO ARRAY CONVERSION (very basic function), MessageTemplate struct is missing
...
// Connected to lrMember(REDIS DATABASE RELATED ISSUES)
...
// DATABASE and STRUCT ISSUES (BonusTemplate, MessageTemplate)
...
func assignGuid(guidType string) string {
	uuidNew := uuid.New()		// to generate universally unique identifier
	switch guidType {
	case "PushMessage":
...
// dynamoSvc, QetAllItems THESE ARE MISSING
...
// THESE THREE ARE NOT IN ANY FUNCTION
