...
func (c *OperationalCounters) incInitialTarget(num int) {
	c.Lock()
	defer c.Unlock()
	c.InitialTarget = c.InitialTarget + num
}

func (c *OperationalCounters) incTargets(num int) {
	c.Lock()
	defer c.Unlock()
	c.Targets = c.Targets + num
}
